...
?? `docker-compose.yml` ????????????????? `6011` ??????? `5001`???????????? `5001`???? `DS2API_HOST_PORT=5001`??????? `ports` ????
...
1. **??/???????`runtime-from-source`?**???????WebUI ?? + Go ?? + ??????
2. **Release ???`runtime-from-dist`?**????????? tag ??? Release ?????? Linux ????? `dist/docker-input/linux_amd64.tar.gz` / `linux_arm64.tar.gz`?Docker ??????????????????? `npm build`/`go build`?

Release ????? Docker ??? release ???????????????????????

????????`/usr/local/bin/ds2api`??????? `5001`?
...
1. **??????**?`PORT` ??? `5001` ???????????????? `http://localhost:8080/admin`??
2. **??????**??????? Docker ?????? Go ??? WebUI ????? `docker compose up -d --build` ????????????????????????
...
- **自动删除会话模式** (`auto_delete.mode`)：默认 `retention`，请求前清空旧的 DS 官方 Web 会话，仅保留本次新会话，并在 10 分钟后删除；也支持 `none` / `single` / `all`。可通过 `PUT /admin/settings` 更新。
