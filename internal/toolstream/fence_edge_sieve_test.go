...
func TestProcessToolSieveOfficialDSMLFenceDoesNotTriggerToolCall(t *testing.T) {
	var state State
	chunks := []string{
		"示例：\n```xml\n",
		"<｜DSML｜tool_calls>\n",
	...
	if toolCalls != 0 {
		t.Fatalf("expected fenced official DSML example to stay text, got %d tool calls events=%#v", toolCalls, events)
	}
	if !strings.Contains(textContent.String(), "示例") || !strings.Contains(textContent.String(), "完毕") {
		t.Fatalf("expected surrounding text preserved, got %q", textContent.String())
	}
